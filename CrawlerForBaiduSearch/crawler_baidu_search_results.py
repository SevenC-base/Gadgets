...
def run(path, keyword, num):
    ...
    # print(tips)
    bar = Bar("Starting the browser driver in about 3 seconds", max=1, fill="█", suffix="%(percent)d%%")
    for i in range(1):
        time.sleep(0.1)
        bar.next()
    bar.finish()
    ...
    #print(type(click_num), click_num)
    ...
    for num in range(1, 11):
        try:
            ...
            try:
                ...
                #print("-------------------------------There is no relevant link !!!")
                break
        finally:
            pass
    if click_num == 1:
    ...
    try:
        ...
        for opt_name, opt_value in opts:
            ...
            if opt_name in ('-v', '--version'):
                print('\n☀☀☀ Crawl Baidu Pages [Version is 3.0] ☀☀☀')
                sys.exit()
        if len(keyword1) > 0:
            # print(len(keyword1))
            #print(num, keyword1)
